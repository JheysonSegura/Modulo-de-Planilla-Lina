...
| A2 | Tokens JWT en cookies sin `httpOnly` | ✅ Arreglado (2026-08-26, sesión dedicada aparte como se recomendaba) — el refresh token ahora vive en una cookie `HttpOnly, SameSite=Strict, Path=/auth` seteada por el backend (`routers/auth.py`), invisible para JavaScript; verificado el header `Set-Cookie` real con curl. El access token quedó solo en memoria del lado del cliente (`useState`, ya no en cookie) — se pierde en cada recarga a propósito, y se repone con un refresh silencioso contra la cookie httpOnly antes de mostrar la app (`useAuth.ts::hidratarSesion`). No hizo falta agregar un proxy/BFF ni un token CSRF aparte: frontend y API son subdominios del mismo sitio registrable, así que `SameSite=Strict` alcanza para bloquear el CSRF clásico. Verificado end-to-end en navegador real (login, recarga con sesión persistente, navegación con datos reales, logout con cookie limpiada) + suite completa (217/217) + lint/typecheck frontend. Encontrado y corregido durante la propia verificación: una cookie `refresh_token` pre-A2 (no httpOnly, `Path=/`) podía convivir con la nueva y causar 401 intermitentes por ambigüedad -- se agregó una limpieza explícita de esa cookie vieja en cada login/refresh/logout, autolimpiándose sola en el primer ciclo de cada usuario tras este cambio. |
...
**Estado final (2026-08-26): A2 arreglado — ver tabla de Críticos/Altos arriba.** El reporte completo queda cerrado en todo lo que es código. Único pendiente real: la emisión del certificado TLS de B6, que depende de que exista un dominio/servidor de producción real (el reverse proxy que lo hará ya está armado, ver `docker-compose.prod.yml`/`scripts/proxy/Caddyfile`).
...
- ~~A2 (rediseño de almacenamiento de tokens a cookie httpOnly)~~ — arreglado 2026-08-26, ver tabla de Críticos/Altos arriba.
