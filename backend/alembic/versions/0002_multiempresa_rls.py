"""Soporte multi-empresa + Row-Level Security (traducción literal de
db/migracion_multiempresa.sql)

Revision ID: 0002_multiempresa_rls
Revises: 0001_schema_base
...
revision: str = "0002_multiempresa_rls"
down_revision: Union[str, None] = "0001_schema_base"
...
def downgrade() -> None:
    op.execute(
        """
        DROP POLICY IF EXISTS empresa_aislamiento_planillas ON planillas;
        DROP POLICY IF EXISTS empresa_aislamiento_contratos ON contratos;
        DROP POLICY IF EXISTS empresa_aislamiento_empleados ON empleados;
        ...
        ALTER TABLE provisiones_vacaciones DROP COLUMN IF EXISTS empresa_id;
        ALTER TABLE provisiones_decimo DROP COLUMN IF EXISTS empresa_id;
        ALTER TABLE liquidaciones DROP COLUMN IF EXISTS empresa_id;
        ALTER TABLE planillas DROP COLUMN IF EXISTS empresa_id;
        ALTER TABLE contratos DROP COLUMN IF EXISTS empresa_id;

        ALTER TABLE empleados DROP CONSTRAINT IF EXISTS uq_empleados_empresa_identificacion;
        ALTER TABLE empleados ADD CONSTRAINT empleados_tipo_identificacion_identificacion_key
