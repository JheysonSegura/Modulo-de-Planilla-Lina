...
- **Implementado** (2026-08-07, `app/services/reportes_service.py` + `app/templates/*.html`): boleta de pago (PDF/Excel) por `movimiento_planilla` — cubre planilla regular y décimo sin código separado, porque el pago de décimo ya genera `Planilla`/`MovimientoPlanilla` reales (`decimo_service.generar_pago_decimo`); boleta de vacaciones (PDF/Excel) por evento; boleta de liquidación (PDF/Excel); exportación de planilla completa (Excel/CSV, `GET /planillas/{id}/exportar`). Todas llevan el membrete de empresa (logo + RUC/DV/dirección/teléfono) vía `_base_boleta.html`.
...
- **Fase 16 (reportes y exportables) implementada 2026-08-07, completa** — ver sección 4 "Reportes y exportables". Boleta de pago/vacaciones/liquidación (PDF/Excel) + exportación de planilla completa (Excel/CSV/PDF) + reporte de auditoría descargable (Excel/CSV), todas con membrete de empresa. 108/108 tests backend en verde, typecheck/lint frontend en verde, verificado con casos reales end-to-end en el navegador (Docker): contrato $2,500/mes + horas extra con boleta cuadrando exacto al neto, export PDF de planilla, export Excel/CSV de auditoría. Sin pendientes de esta fase.
